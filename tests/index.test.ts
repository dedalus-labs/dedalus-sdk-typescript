...
describe('logging', () => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Dedalus({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'DEDALUS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
...
describe('idempotency', () => {
  test.skip('key can be set per-request', async () => {
    const client = new Dedalus({ baseURL: process.env["TEST_API_BASE_URL"] ?? 'http://127.0.0.1:4010', apiKey: 'My API Key' });
    await client.embeddings.create({ input: 'string', model: 'text-embedding-ada-002' }, { idempotencyKey: 'my-idempotency-key' })
  })
})
