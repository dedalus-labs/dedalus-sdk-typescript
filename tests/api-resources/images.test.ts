...
describe('resource images', () => {
  ...
  test.skip('createVariation: only required params', async () => {
    const responsePromise = client.images.createVariation({ image: await toFile(Buffer.from('Example data'), 'README.md') });
  ...
  test.skip('edit: only required params', async () => {
    const responsePromise = client.images.edit({ image: await toFile(Buffer.from('Example data'), 'README.md'), prompt: 'prompt' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
