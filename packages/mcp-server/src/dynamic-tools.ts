...
export function dynamicTools(endpoints: Endpoint[]): Endpoint[] {
  ...
  const listEndpointsTool = {
    ...
    tool: {
      ...
      description: 'List or search for all endpoints in the Dedalus TypeScript API',
      inputSchema: zodToInputSchema(listEndpointsSchema),
    },
    handler: async (client: Dedalus, args: Record<string, unknown> | undefined): Promise<ToolCallResult> => {
  ...
  const getEndpointTool = {
    ...
    tool: {
      ...
      description:
        'Get the schema for an endpoint in the Dedalus TypeScript API. You can use the schema returned by this tool to invoke an endpoint with the `invoke_api_endpoint` tool.',
      inputSchema: zodToInputSchema(getEndpointSchema),
    },
    handler: async (client: Dedalus, args: Record<string, unknown> | undefined) => {
      if (!args) {
  ...
  const invokeEndpointTool = {
    ...
    tool: {
      ...
      description:
        'Invoke an endpoint in the Dedalus TypeScript API. Note: use the `list_api_endpoints` tool to get the list of endpoints and `get_api_endpoint_schema` tool to get the schema for an endpoint.',
